module NavigationHelpers
  ...
  def path_to(page_name)
    ...
    when /the "(.+)" (computer|peripheral) images page/
      send("admin_#{$2}_images_path", $2.capitalize.constantize.find_by_name($1))
    when /the "(.+)" (computer|peripheral) keywords page/
      send("admin_#{$2}_ebay_keywords_path", $2.classify.constantize.find_by_name($1))
    else
